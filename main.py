...
class DiamondCatalogProcessor:
    ...
    def _parse_price_number(value: str) -> float:
        """Extract numeric part from '123.45 CAD' or return float(value) if plain number."""
        if value is None:
            return 0.0
        s = str(value).strip()
    ...
    def clean_and_validate_data(self, df: pd.DataFrame, product_type: str) -> pd.DataFrame:
        ...
        if 'image' in df.columns:
            df['image'] = df['image'].str.extract(r'(https?://[^\s]+\.(?:jpg|jpeg|png|webp))', expand=False)
            df['image'] = df['image'].fillna('')
            initial_count = len(df)
            df = df[df['image'].str.len() > 0]
            logger.info(f"Filtered {initial_count - len(df)} rows without valid images")

        # Clean and validate price (raw 'price' if present; we later overwrite with CAD)
    ...
    def process_file(self, file_path: str, product_type: str) -> pd.DataFrame:
        ...
        try:
            ...
            df['markupPrice'] = df['markupPrice'].apply(self.convert_to_cad)

            df['price'] = df['markupPrice'].map(lambda x: f"{x:.2f} CAD")
            df['markupCurrency'] = 'CAD'

            # Generate unique IDs
            if 'ReportNo' in df.columns:
        ...
        product_templates = {
            "natural": {
                ...
                "description": lambda row: (
                    ...
                    f"Perfect for custom jewelry creations. Measurements: {row.get('length', '')}-{row.get('width', '')}x{row.get('height', '')} mm. "
            ...
            "lab_grown": {
                ...
                # SEO-friendly link: shape + carat + color + clarity + lab + reportno
                "link": lambda row: (
            ...
            "gemstone": {
                ...
                "description": lambda row: (
                    ...
                    f"Perfect for custom jewelry designs."
                ),
    ...
    def create_country_specific_files(self, combined_df: pd.DataFrame) -> None:
        """Create country-specific files with accurate currency conversion (from CAD to target)."""
        ...
        usd_to_cad = Decimal(str(rates.get("CAD", 1)))  # USD->CAD rate
        if usd_to_cad == 0:
            usd_to_cad = Decimal("1")
        ...
        for country, currency in self.country_currency.items():
            try:
                ...
                # Update IDs for the country (replace trailing CA with country code)
                country_data['id'] = country_data['id'].str.replace(r'CA$', country, regex=True)

                # Convert from CAD price -> target currency
                target_rate = Decimal(str(rates[currency]))  # USD->target
                # CAD->target = (USD->target) / (USD->CAD)
                cad_to_target = (target_rate / usd_to_cad)

                # Parse numeric CAD and convert
                ...
                # Format price with target currency code
                country_data['price'] = country_data['price_numeric'].map(lambda x: f"{x:.2f} {currency}")
